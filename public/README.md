...
2. **GPS 定位** —— 取得每個地點的經緯度，並標示在地圖上。
3. **合理性檢查** —— 相鄰地點的實際道路距離超過 **20 公里** 視為不合理，自動標記並排除於總計之外。
4. **地點介紹 + 語音導覽** —— 擷取自然、人文、歷史、地形、地理、美食、工藝等介紹，並以瀏覽器語音朗讀。
5. **實際道路路線與坡度** —— 以單車／道路路網規劃實際可騎路線，列出每段距離、爬升、坡度剖面；坡度超過 **8%** 特別標註，並標出最陡坡度。
6. **難度比較** —— 與台灣大眾單車路線（淡水河左岸、日月潭環潭、風櫃嘴、武嶺等）比較難度等級。
...
2. 按「辨識地點」，系統會列出自動辨識的地點清單。
3. 在清單中檢查、修正錯字、刪除非地點項目、補上漏掉的地點或調整順序（一行一個）。查不到的地名可寫得更完整，例如「基隆」→「基隆火車站」。
4. 按「確認地點，開始規劃」，即會依序產生地圖、地點導覽、路段坡度與難度比較。
